...
//        selectRandomButton.onClick.AddListener(OnSelectRandomButtonClicked);
...
//    {
//        if (isCombiningModeActive || isSellingModeActive) return; // 이미 합성 모드이거나 판매 모드일 경우 아무 동작도 하지 않음
...
//    {
//        if (isSellingModeActive || isCombiningModeActive) return; // 이미 판매 모드이거나 합성 모드일 경우 아무 동작도 하지 않음
